  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (result.success) {
      toast({
        ...
        description: `Successfully registered a new wellness order.`,
      });
      setOpen(false);
      form.reset();
    } else {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl rounded-3xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-bold text-primary">Initiate Sales Order</DialogTitle>
          <DialogDescription>
            Register a confirmed product order for a wellness contact.
          </DialogDescription>
        </DialogHeader>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 pb-4 pt-2">
                    ...
                    <FormField control={form.control} name="contactId" render={({ field }) => (
                        <FormItem>
                          ...
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger disabled={isLoading}><SelectValue placeholder="Select contact" /></SelectTrigger></FormControl>
                            <SelectContent>
                ...
                <FormField
                  ...
                  render={() => (
                    <FormItem className="space-y-4">
                      <FormLabel className="font-bold text-primary">Fulfilment List</FormLabel>
                          ...
                          <div key={field.id} className="flex items-center gap-2 p-2 border rounded-xl bg-muted/20">
                            <div className="grid grid-cols-3 gap-2 flex-grow">
                                <Select onValueChange={(value) => handleProductChange(index, value)} defaultValue={field.productId}>
                                    <SelectTrigger className="col-span-3 h-9 rounded-lg"><SelectValue placeholder="Configure Package" /></SelectTrigger>
                      ...
                      <Button type="button" variant="outline" size="sm" className="w-full border-dashed rounded-xl" onClick={() => append({productId: '', productName: '', quantity: 1, unitPrice: 0, subtotal: 0})}>
                        <PlusCircle className="mr-2 h-4 w-4" /> Add Ayurvedic Product
                      </Button>
                      <FormMessage />
                    </FormItem>
                  )}
              ...
              <Button type="submit" disabled={isSubmitting || isLoading} className="w-full h-12 rounded-xl herbal-gradient font-bold shadow-lg">
                {isSubmitting ? 'Finalizing Fulfilment...' : 'Finalize Sales Order'}
              </Button>
            </form>
          </Form>
        </ScrollArea>
